...
const Cards = ({card}) => {
    ...
    return (
        ...
        <div className="card-body   text-black">
          ...
          <p className='flex gap-2 items-center'>Rating: <IoStar className='text-yellow-500'></IoStar> {rating}</p>
          <p>{description.length>200?description.slice(0,100):description}</p>
          <div className="card-actions ">
            <Link to={`/cardViewDetails/${_id}`}><button className="btn btn-primary text-black bg-yellow-500 border-none">View Details</button></Link>
          </div>
        </div>
      </div>
    );
